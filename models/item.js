...
module.exports = class Item extends Sequelize.Model {
   ...
   static associate(db) {
      ...
      Item.belongsToMany(db.Order, { through: db.OrderItem, foreignKey: 'itemId', otherKey: 'orderId' }) // 교차테이블 관계 설정

      Item.hasMany(db.CartItem, { foreignKey: 'itemId', sourceKey: 'id', onDelete: 'CASCADE' })
      Item.belongsToMany(db.Cart, { through: db.CartItem, foreignKey: 'itemId', otherKey: 'cartId' }) // 교차테이블 관계 설정
   }
}
